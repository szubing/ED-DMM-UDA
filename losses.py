...
def center_loss(tgt_model, batch, src_model, src_centers, tgt_centers, 
    ...
    f_N = tgt_model.get_embedding(batch["X"].cuda())
...
### K-neighbors
    # embeddings = np.zeros((n_samples, 72))
    # for images, target in dataloader:
        # with torch.no_grad():
            # embeddings[k:k+len(images)] = images.numpy().reshape(-1,72)
            # labels[k:k+len(images)] = target.numpy()
            # k += len(images)
...
def get_neg_hard(neg_ind, 
    ...
    for ap_i, ap_di in zip(ap, ap_D):
        ...
        loss_values = (ap_di -D[np.array([ap_i[0]]), neg_ind] + margin)
