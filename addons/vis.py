...
from sklearn import datasets as sl_datasets
...
def visEmbed(exp_dict):
    ...
    src_model.load_state_dict(torch.load(exp_dict["path"]+"/model_src_run{}.pth".format(exp_dict['run'])))
    ...
    tgt_model, tgt_opt, tgt_scheduler = models.get_model(exp_dict["tgt_model"], 
                                          ...
                                          patch_size=exp_dict['patch_size'],
                                          n_classes=exp_dict['n_classes'])
    tgt_model.load_state_dict(torch.load(exp_dict["path"]+"/model_tgt_run{}.pth".format(exp_dict['run'])))
    ...
    if 1:
        ...
        plt.savefig("figures/src_{}_center_{}_disc_{}.pdf".format(exp_dict["exp_name"].replace(" ",""),exp_dict["options"]["center"],exp_dict["options"]["disc"]),
                bbox_inches='tight', 
               transparent=False)

        plt.savefig("figures/src_{}_center_{}_disc_{}.png".format(exp_dict["exp_name"],exp_dict["options"]["center"],
    ...
    if 1:
        ...
        plt.title(title)
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig("figures/tgt_{}_center_{}_disc_{}.pdf".format(exp_dict["exp_name"],exp_dict["options"]["center"],
        exp_dict["options"]["disc"]),
                bbox_inches='tight', 
               transparent=False)

        plt.savefig("figures/tgt_{}_center_{}_disc_{}.png".format(exp_dict["exp_name"],exp_dict["options"]["center"],
        exp_dict["options"]["disc"]),
                bbox_inches='tight', 
               transparent=False)
        # ms.visplot(fig)
    ...
    # n_points = 1000
    # X, color = sl_datasets.samples_generator.make_s_curve(n_points, random_state=0)
    # n_neighbors = 10
    # n_components = 2
...
def scatter_source(src_model, tgt_model, src_loader, 
    ...
    plt.savefig("figures/{}.pdf".format(fname))
    plt.close()
